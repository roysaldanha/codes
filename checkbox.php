...
<?php 
    ...
    function woof_draw_checkbox_childs($taxonomy_info, $tax_slug, $term_id, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos)
    {
        ...
        if ($WOOF->is_isset_in_request_data($tax_slug))
        {
            ...
            if(isset($product_cats[0]->slug) && !empty($product_cats[0]->slug)){
                $current_request[0] = $product_cats[0]->slug;
            } else {
                $current_request = array();
            }
        }
        ...
        <?php if (!empty($childs) AND is_array($childs)): ?>
            <ul class="woof_childs_list woof_childs_list_<?php echo $term_id ?>" <?php if ($hide_childs == 1 || 
                !in_array($current_request[0], $uniquechilds) || empty($current_request[0])): ?>style="display: none;"<?php endif; ?>>
                <?php foreach ($childs as $term) : $inique_id = uniqid(); ?>
                    <?php //echo "<pre>";print_r($term);
                    $count_string = "";
                    $count = 0;
                    ...
                    if($current_request[0] == $term['slug']) { ?>
                    <script type="text/javascript"> 
                    ...
                    ?> 
                    <li class=" woof_li_term_<?php echo $term['term_id'] ?>" <?php if ($WOOF->settings['dispay_in_row'][$tax_slug] AND empty($term['childs'])): ?>style="display: inline-block !important;"<?php endif; ?>><input type="checkbox" <?php if (!$count AND ! in_array($term['slug'], $current_request) AND $show_count): ?>disabled=""<?php endif; ?> id="<?php echo 'woof_' . $term['term_id'] . '_' . $inique_id ?>" class="woof_checkbox_term woof_checkbox_term_<?php echo $term['term_id'] ?>" data-tax="<?php echo $tax_slug ?>" name="<?php echo $term['slug'] ?>" data-term-id="<?php echo $term['term_id'] ?>" value="<?php echo $term['term_id'] ?>" <?php echo checked(in_array($term['slug'], $current_request)|| empty($current_request[0])) ?> /><label class="woof_checkbox_label <?php if (in_array($term['slug'], $current_request)): ?>woof_checkbox_label_selected<?php endif; ?>" for="<?php echo 'woof_' . $term['term_id'] . '_' . $inique_id ?>"><?php
